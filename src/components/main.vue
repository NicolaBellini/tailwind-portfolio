<script>
export default {};
</script>

<template>
  <div class="px-10 md:px-20 lg:px-40 dark:bg-slate-900 dark:text-white">
    <div class="pt-10 translate-y-[100px]">
      <h3 class="text-3xl py-2">Services I offer</h3>
      <p class="text-md py-2 leading-8 text-gray-700 dark:text-white mb-10">
        Lorem ipsum dolor sit amet consectetur adipisicing elit. Doloribus
        possimus quas totam repellendus,<span class="text-teal-500">
          facere ullam unde</span
        >
        similique? Eum vitae nemo tempora,<span class="text-teal-500">
          facere ullam unde</span
        >
        repellendus quam hic. Aperiam autem soluta fugit quis illum!
      </p>
    </div>
    <div class="lg:flex gap-5 pt-[100px]">
      <div
        class="text-center shadow-xl p-10 rounded-xl py-10 lg:w-1/3 mb-10 dark:bg-teal-400"
      >
        <img src="/design.png" alt="" class="w-32 mx-auto" />
        <h3 class="text-lg font-medium pt-8 pb-2">Beautiful design</h3>
        <p class="py-2">
          Lorem ipsum dolor sit amet consectetur adipisicing elit. Dolores
          dolor, dicta aliquam qua
        </p>
        <h4 class="text-teal-600 py-4">design tools i use</h4>
        <p class="text-gray-800 py-1 dark:text-white">photoshop</p>
        <p class="text-gray-800 py-1 dark:text-white">adobe illustator</p>
        <p class="text-gray-800 py-1 dark:text-white">figma</p>
      </div>
      <div
        class="text-center shadow-xl p-10 rounded-xl py-10 mb-10 lg:w-1/3 dark:bg-teal-400"
      >
        <img src="/code.png" alt="" class="w-32 mx-auto" />
        <h3 class="text-lg font-medium pt-8 pb-2">Beautiful design</h3>
        <p class="py-2">
          Lorem ipsum dolor sit amet consectetur adipisicing elit. Dolores
          dolor,
        </p>
        <h4 class="text-teal-600 py-4">design tools i use</h4>
        <p class="text-gray-800 py-1 dark:text-white">photoshop</p>
        <p class="text-gray-800 py-1 dark:text-white">adobe illustator</p>
        <p class="text-gray-800 py-1 dark:text-white">figma</p>
      </div>
      <div
        class="text-center shadow-xl p-10 rounded-xl py-10 lg:w-1/3 lg:mb-10 dark:bg-teal-400"
      >
        <img src="/consulting.png" alt="" class="w-32 mx-auto" />
        <h3 class="text-lg font-medium pt-8 pb-2">Beautiful design</h3>
        <p class="py-2">
          Lorem ipsum dolor sit amet consectetur adipisicing elit. Dolores
          dolor, dicta aliquam quam veniam enim error quod debitis laboriosam
          quidem.
        </p>
        <h4 class="text-teal-600 py-4">design tools i use</h4>
        <p class="text-gray-800 py-1 dark:text-white">photoshop</p>
        <p class="text-gray-800 py-1 dark:text-white">adobe illustator</p>
        <p class="text-gray-800 py-1 dark:text-white">figma</p>
      </div>
    </div>
  </div>
</template>

<style></style>
